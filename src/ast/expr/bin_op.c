...
static void* ast_bin_op_accept_transformer(void* self_, ast_transformer_t* transformer, void* out);
static void ast_bin_op_destroy(void* self_);

static ast_node_vtable_t ast_bin_op_vtable =
{
    .accept = ast_bin_op_accept,
    .accept_transformer = ast_bin_op_accept_transformer,
...
{
    ast_bin_op_t* self = self_;
    return transformer->transform_bin_op(transformer, self, out);
}
